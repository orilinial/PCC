...
class Sender():
    ...
    def can_send_packet(self):
        return self.packets_in_flights < self.cwnd
    ...
    def on_packet_acked(self, rtt):
        ...
        if self.cwnd < self.sshthresh:  # slow start
    ...
    def on_packet_lost(self):
        ...
        self.packets_in_flights -= 1
        self.bytes_in_flight -= BYTES_PER_PACKET
        self.sshthresh = int(self.cwnd / 2) + 1
...
class SimulatedNetworkEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.links = None
        self.senders = None
        self.create_new_links_and_senders(max_bw)
    ...
    def step(self, action):
        ...
        for sender in self.senders:
            sender.record_run()
            if sender.cwnd >= sender.sshthresh:
                sender.cwnd += 1
            # print('Step %d: CWND=%d, SSHTHRESH=%d' % (self.steps_taken, sender.cwnd, sender.sshthresh))
        self.steps_taken += 1
    ...
    def create_new_links_and_senders(self, max_bw):
...
register(id='PccNsTCP-v0', entry_point='network_sim_tcp_cc:SimulatedNetworkEnv')
