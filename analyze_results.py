...
def throughput_vs_send_rate(rl, path):
    res_bws = []
    max_bws = []
    for bw in rl:
...
def throughput_of_modified_reward(rl_dict, rl_latency_dict, path):
    ...
    plt.savefig(path + 'throughput_of_modified_reward.png')
...
def analyze():
    ...
    test_rl_latency = {10.0: parse_json('results/test_rl_latency_10.00.json')}

    throughput_vs_send_rate(test_rl, graphs_dir)
    throughput_rl_vs_tcp_cc(test_rl, test_tcp_cc, graphs_dir)
    latency_and_loss_rl_graph(test_rl, graphs_dir)
    throughput_of_modified_reward(test_rl, test_rl_latency, graphs_dir)  # Throughputs of RL-throughput bs RL-latency
