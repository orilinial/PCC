...
class Network():
    ...
    def __init__(self, senders, links, reward_type):
        self.q = []
        self.cur_time = 0.0
    ...
    def run_for_dur(self, dur):
        ...
        if self.reward_type == 'latency':
            reward = 2.0 * throughput / (8 * BYTES_PER_PACKET) - 2e3 * latency - 2e3 * loss
        else:
        ...
        # Low latency
        # reward = 2.0 * throughput / (8 * BYTES_PER_PACKET) - 4e3 * latency - 2e3 * loss
...
class SimulatedNetworkEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.links = None
        self.senders = None
        self.create_new_links_and_senders(bw, test)
        self.net = Network(self.senders, self.links, reward_type)
        self.run_dur = None
        self.run_period = 0.1
        self.steps_taken = 0
    ...
    def step(self, actions):
        # if self.steps_taken == int(MAX_STEPS/3):
        #     self.links[0].bw *= 2
        #     self.links[1].bw *= 2
        #
        # if self.steps_taken == int(2 * MAX_STEPS/3):
        #     self.links[0].bw /= 2
        #     self.links[1].bw /= 2
    ...
    def create_new_links_and_senders(self, max_bw, test):
        ...
        if test:
            sender_init_bw = 0.3 * bw
        else:
            sender_init_bw = random.uniform(0.3, 1.5) * bw
        self.senders = [Sender(sender_init_bw, [self.links[0], self.links[1]], 0, self.features, history_len=self.history_len)]
        self.run_dur = 3 * lat

    def reset(self, reward, max_bw, test=False):
        self.steps_taken = 0
        self.net.reset()
        self.create_new_links_and_senders(max_bw, test)
        self.net = Network(self.senders, self.links, reward)
        self.episodes_run += 1
